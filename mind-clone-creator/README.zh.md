...
1. 人格层：判断标准、边界意识、表达风格、协作偏好
2. 工具层：真实环境里有哪些工具、技能，以及它们各自能做什么
3. 工作流层：任务如何分阶段推进，每个阶段要什么输入、产出什么输出
4. 决策层：当前处于哪个阶段、下一步该做什么、什么时候继续、什么时候停下

默认流程一定先从人格层开始。
只有在明确了具体工作单元之后，`persona-plus-workflow` 才会继续往上编译工作流层和决策层。
...
换句话说，默认交付会先稳稳落在人格层。
更高的工作流层和决策层，需要额外的 workflow 建模才能成立。
