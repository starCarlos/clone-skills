...
把“分身 / clone / 顾问型人格”从一次性的 prompt，做成有思维层级、有边界、可复用的技能工作流。
...
| Skill | 核心层级 | 适合构建什么 |
| --- | --- | --- |
| `mind-clone-creator` | 人格层 -> 工具层 -> 工作流层 -> 决策层 | 基于你自己的经验、判断方式、工作习惯和表达风格构建 clone |
| `mind-clone-advisor` | 语料与合规层 -> 画像与 Prompt 层 -> 图谱层 | 基于他人的公开资料构建合规的 advisor workflow |
...
这个仓库的核心判断是：一个真正有用的 clone，不能只像“说话口吻”，还要把思维是如何分层组织的讲清楚。

- 如果你只要语气模仿，一段 prompt 可能就够了
- 如果你想复用判断方式、工具调用方式、工作流行为，甚至决策方式，就必须把层级显式建出来
- 如果你希望后续还能继续审查、修订、扩展 clone，思维层级就是最重要的骨架

所以这里做的不是单点 persona 包装，而是把思维克隆拆成可解释、可升级的层级结构。
...
- 判断你要做的到底是语气 clone、判断 clone、工作流 clone，还是决策辅助 clone
